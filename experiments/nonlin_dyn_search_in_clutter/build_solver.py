...
import numpy as onp
from time_opt_erg_lib.dynamics import DoubleIntegrator, SingleIntegrator, KinematicBicycle
...
def build_erg_time_opt_solver(args):
    ...
    def ineq_constr(params, args):
        ...
        _ctrl_box = [(-u[:,0]+1.0).flatten(), (u[:,0]-5.0).flatten(), (np.abs(u[:,1]) - np.pi/3).flatten()]
